...
export function Product() {
  ...
  if (!product) {
    return (
      <section>
        <p>Товар не найден.</p>
        <Link to="/catalog" className={styles['product-page__back-link']}>
          Вернуться в каталог
        </Link>
      </section>
    );
  ...
  return (
    <section className={styles['product-page']}>
      ...
      <div className={styles['product-page__content']}>
        ...
        <p className={styles['product-page__price']}>{formatPrice(product.price)}</p>
        <div className={styles['product-page__actions']}>
          {isAdmin ? (
            <>
              <Link to={`/product/${product.id}/edit`} className={styles['product-page__button']}>
                Редактировать
              </Link>
              <Link
                to={`/product/${product.id}/reviews`}
                className={`${styles['product-page__button']} ${styles['product-page__button--secondary']}`}
              >
                Отзывы
              </Link>
              <button
                type="button"
                className={`${styles['product-page__button']} ${styles['product-page__button--danger']}`}
            ...
            <>
              <button type="button" className={styles['product-page__button']} onClick={() => addItem(product)}>
                В корзину
              </button>
              <Link
                to={`/product/${product.id}/reviews`}
